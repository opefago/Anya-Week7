...
import { moveBlocks } from "../block-utils"
...
export async function propose(args: any[], functionToCall: string, proposalDescription: string) {
  ...
  console.log(`Proposing ${functionToCall} on ${controller.address} with ${args}`)
...
propose([NEW_REWARDS_VALUE], FUNC, PROPOSAL_DESCRIPTION)
  .then(() => process.exit(0))
  .catch((error) => {
    console.error(error)
    process.exit(1)
